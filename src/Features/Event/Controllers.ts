...
import { newEventBody, patchEventBody, eventIdParams, userIdParams, inviteUsersRequest, userIdEventIdParams } from "./Contracts";
    ...
    await Promise.all(
      req.body.map(async function (userId) {
        ...
        if (user) {
          ...
          usersAdds.push(userId);
        }
      })
    );

    return res.status(200).send(usersAdds);
  } catch (error) {
    return res.status(500).send(error);
  }
};
